...
numChannelRealizationsPerEpisode = 250;  %2500 leads to a 5 GB file and -v6 has a limit of 2 GB
...
for ms_loc_realization_idx =...
    ...
    if 0
        ...
        %Use AR(1) for channel time evolution, from:
        ...
        if 1 %use 1 to directly specify the normalized Doppler shift
            fDTs = 0.5; %normalized Doppler shift
        else
            fc = 3.5e9; %carrier frequency (Hz)
            c = 3e8; %speed of light (m/s)
            v = 3; %user velocity (m/s)
            fD = (v*fc)/c; %maximum Doppler shift (Hz)
            Ts = 1e-3; %channel sampling duration (s)
            fDTs = fD*Ts; %normalized Dopper shift
        end
        %for AR(1), the filter coefficient coincides with aucorrelation at k=1
        alpha = besselj(0,2*pi*fDTs);
        
        %Need the initial channel to implement Eq. (17) in [1]
        %To do, read reference [25] in [1]:
        %A. Forenza, D. J. Love, and R.W. Heath, Jr., “Simplified spatial correlation
        %models for clustered MIMO channels with different array configurations,”
        %IEEE Trans. Veh. Technol., vol. 56, no. 4, pp. 1924–1934, July 2007
    end
