...
class Buffers:
    ...
    def packets_departure_drop(self, num_candidate_packets_to_be_discarded, buffer_id):

        num_removed_packets = num_candidate_packets_to_be_discarded
        i = buffer_id

        if num_removed_packets > self.buffer_occupancies[i]:
            num_removed_packets = self.buffer_occupancies[i]
            self.buffer_occupancies[i] = 0
        else:
            self.buffer_occupancies[i] -= num_removed_packets
            #update packets_arrival_time
        this_packets_arrival_time = self.packets_arrival_time[i] #get numpy array from list
        num_remaining_packets = int(self.buffer_sizes[i] - num_removed_packets)
        this_packets_arrival_time[0:num_remaining_packets]=this_packets_arrival_time[num_removed_packets:]
        this_packets_arrival_time[num_remaining_packets:]=self.age_value_when_buffer_is_empty #np.NaN #need to initialize the time for the earliest packets
        self.packets_arrival_time[i] = this_packets_arrival_time
        return num_removed_packets
    ...
    def packets_arrival(self, num_incoming_packets):
        ...
        for i in range(self.num_buffers):
            #checking oldest packages to be dropped
            this_packets_arrival_time = self.packets_arrival_time[i]  # get numpy array from list

            #this_packet_age = [int((self.time_counter-1) - pi) for pi in this_packets_arrival_time if pi != 0]
            this_packet_age = (self.time_counter - 1) - this_packets_arrival_time
            #this_packet_age = [int(self.time_counter - 1 - pi) for pi in this_packets_arrival_time]
            #old_pkts_to_be_dropped = np.split(this_packet_age, np.where(this_packet_age[:-1] == self.max_packet_age)[0])
            #old_pkts_to_be_dropped = list(map(lambda x: x if x>=self.max_packet_age else None, this_packet_age))
            old_pkts_to_be_dropped = []
            mam = np.argmax(this_packet_age>=self.max_packet_age)
            if mam > 0:
                old_pkts_to_be_dropped = this_packet_age[mam:-1]
            #old_pkts_to_be_dropped = np.array([a for a in this_packet_age if a >= self.max_packet_age])
            if len(old_pkts_to_be_dropped) > 0:
                drop_oldests = self.packets_departure_drop(len(old_pkts_to_be_dropped), buffer_id=i)
                self.dropped_last_iteration[i] += drop_oldests
                self.dropped_counter[i] += drop_oldests
            ...
            if added_packets != num_incoming_packets[i]:
                ...
                self.dropped_last_iteration_percentual[i] = self.dropped_last_iteration[i] / self.buffer_occupancies[i]
