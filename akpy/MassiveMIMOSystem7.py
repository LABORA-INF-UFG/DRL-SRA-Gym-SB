'''
From Version 5 -> runing the Ryan's dataset
...
class MassiveMIMOSystem:
    ...
    def convert_H_into_Hhat(self,H,debugme):
        ...
        if debugme == True: #match other functions, for debugging
            ...
            Np = np.sqrt(0.5)*(np.random.randn(self.num_bs_antennas, nbrOfRealizations, self.num_UEs_per_BS, self.L, self.pilot_reuse_factor) +
                               1j * np.random.randn(self.num_bs_antennas, nbrOfRealizations, self.num_UEs_per_BS, self.L, self.pilot_reuse_factor))
    ...
    def SE_agora_vai(self, selected_users, H, Hhat, Kmax):
        ...
        #Prepare to store simulation results for signal gains
        #signal_MR = np.zeros((self.num_UEs_per_BS, self.L), dtype=np.complex64)
        signal_MR = np.zeros((len(selected_users),), dtype=np.complex128)
        intracell_interference = np.zeros((len(selected_users),)) #real number

        #Prepare to store simulation results for Bernoulli_extra_interference powers
        #interf_MMMSE = np.zeros((self.K,self.L,self.K,self.L),dtype=np.complex64)
        this_intercell_interference = self.current_intercell_interference
        this_intercell_interference = this_intercell_interference[selected_users] #keep only for selected_users
        ...
        this_interference = intracell_interference + self.rhoEqual * this_intercell_interference
        #prelogFactor = self.tau_d/self.tau_c
    ...
    def load_episode(self, episode_number):
        ...
        #loaded_H = data_dict.channel.H
        loaded_H = data_dict['H']
        self.H = loaded_H

        # TODO - compute Hhat
        self.Hhat = copy.deepcopy(self.H)
    ...
    def SE_for_given_sample(self, current_sample_index, selected_users, Kmax, avoid_estimation_errors = False):
        ...
        #ALERTA!!! Alteração feita para usar o dataset do Ryan, sem ter a Hhat!!!!! Utilize a instrução anterior ^^^^
        b = current_sample_index

        #thisHhat = np.transpose(self.Hhat[:, b, :])
        #thisH = np.transpose(self.H[:, current_sample_index, :])
