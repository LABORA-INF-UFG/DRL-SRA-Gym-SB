...
class SRAEnv(gym.Env):
    ...
    def step(self,action):
        ...
        self.rates_pkt_per_s = self.rateEstimationUsers([self.F[self.curr_freq]], [[action]], self.mimo_systems, self.K,
                                                            self.curr_slot, self.packet_size_bits)

        se = self.updateSEUsers([self.F[self.curr_freq]], [[action]], self.mimo_systems,
                                    self.recent_spectral_eff, self.curr_slot)

        reward, _ = self.rewardCalc([self.F[self.curr_freq]], [[action]], self.mimo_systems, self.K, self.curr_slot,
                                      self.packet_size_bits, [self.rates_pkt_per_s, rates], self.buffers,
                                      self.min_reward, se, update=False)

        if (self.curr_freq == (len(self.F)-1)) and (len(self.alloc_users[self.curr_freq]) == self.F[self.curr_freq]):
            ...
            self.rewardCalc(self.F, self.alloc_users, self.mimo_systems, self.K, self.curr_slot,
                            self.packet_size_bits, [self.rates_pkt_per_s, rates], self.buffers,
